...
IS_POSSESSIVE = compile(
    r"{alnum}+(?:{hyphen}{alnum}+)*(?:{apo}[sS]|[sS]{apo})$".format(
        alnum=ALNUM, hyphen=HYPHEN, apo="['" + APOSTROPHE[1:]
    ), UNICODE
)
"""A pattern that matches English words with a possessive s terminal form."""

IS_CONTRACTION = compile(
    r"{alnum}+(?:{hyphen}{alnum}+)*{apo}(?:d|ll|m|re|s|t|ve)$".format(
        alnum=ALNUM, hyphen=HYPHEN, apo="['" + APOSTROPHE[1:]
    ), UNICODE
)
...
def symbol_tokenizer(sentence):
    ...
    """
    return [token for span in space_tokenizer(sentence) for token in symbol_tokenizer.split(span) if token]
...
def word_tokenizer(sentence):
    ...
    tokens = [token for span in space_tokenizer(
        pruned) for token in word_tokenizer.split(span) if token]
...
def web_tokenizer(sentence):
    ...
    """
    return [
        token for i, span in enumerate(web_tokenizer.split(sentence))
...
def main():
    ...
    parser = ArgumentParser(
        ...
        epilog='default tokenizer: token; default encoding: ' + getdefaultencoding()
    )
    parser.add_argument(
        'files', metavar='FILE', nargs='*', help='One-Sentence-Per-Line file; if absent, read from STDIN'
    )
    parser.add_argument(
        '--possessive-marker',
        '-p',
    ...
    mode.add_argument('--space', '-s', action='store_const',
                      dest='mode', const=SPACE, help=space_tokenizer.__doc__)
    mode.add_argument('--alnum', '-a', action='store_const',
                      dest='mode', const=ALNUM, help=symbol_tokenizer.__doc__)
    mode.add_argument('--token', '-t', action='store_const',
                      dest='mode', const=TOKEN, help=word_tokenizer.__doc__)
    mode.add_argument('--web', '-w', action='store_const',
                      dest='mode', const=WEB, help=web_tokenizer.__doc__)
    ...
    if args.split_contractions:
        ...
        def tokenizer(sentence):
            return split_contractions(tokenizer_func(sentence))
    elif args.possessive_marker:

        def tokenizer(sentence):
            return split_possessive_markers(tokenizer_func(sentence))
    else:
